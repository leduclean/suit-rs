...
fn test_suit_verify_mac0() {
    ...
    let mac_bytes = cbor_diag::parse_diag(include_str!("diags/mac0_hs256_64.edn"))
        .unwrap()
        .to_bytes();
    let mac: CoseMac0 = minicbor::decode(&mac_bytes).expect("Unvalid format for CoseSign1");
